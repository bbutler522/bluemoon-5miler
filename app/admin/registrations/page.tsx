...
const statusStyles: Record<string, { icon: typeof CheckCircle2; color: string; bg: string }> = {
  ...
  refunded: { icon: XCircle, color: 'text-stardust/100', bg: 'bg-stardust/5' },
};
                  ...
                  >
                    <span className="flex items-center gap-1">
                      ...
                      {col.key && sort === col.key && (
                        <ArrowUpDown size={10} className="text-stardust/100" />
                      )}
                    </span>
                  </th>
                ))}
              ...
              {loading ? (
                ...
                registrations.map((reg) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-stardust/100">
                        {reg.bib_number ? `#${reg.bib_number}` : '—'}
                      </td>
                      <td className="px-4 py-3 text-moonlight font-medium whitespace-nowrap">
                        {reg.first_name} {reg.last_name}
                      </td>
                      <td className="px-4 py-3 text-stardust/100">{reg.email}</td>
